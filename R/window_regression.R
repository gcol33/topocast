...
#' predictors over a square moving window, and returns full-resolution intercept
#' and slope grids. The fit uses summed-area tables, so the cost does not grow
#' with the window size. Cells are excluded from a window where the response or
#' any predictor is non-finite; a cell is returned as `NA` when its window holds
#' fewer valid cells than the model needs or a predictor has no spread.
#'
#' This is the matrix engine behind [topocast()]; it works on plain numeric
#' matrices and does not depend on terra.
#'
#' @param y Numeric matrix, the response on the coarse grid.
...
#'
#' @return A list with `intercept` (a numeric matrix) and `slope` (a list of
...
window_regression <- function(y, x, radius, min_cells = 0L, min_variance = 1e-8) {
  ...
  window_regression_cpp(y, x, as.integer(radius),
